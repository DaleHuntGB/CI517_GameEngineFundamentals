...
void GraphicsEngine::UseGUIFont(TTF_Font* _GUIFont) {
	if (nullptr == GUIFont) {
#ifdef __DEBUG
		debug("GraphicsEngine::UseGUIFont()", "font is null");
#endif
		return;
	}
...
void GraphicsEngine::DrawGUIText(const std::string& text, const int& x, const int& y, const int textFontSize) 
{
	SDL_Texture* textTexture = createTextureFromString(text, GUIFont, drawColor);
	int w, h;
	SDL_QueryTexture(textTexture, 0, 0, &w, &h);
	SDL_Rect dst = { x, y, w, h };
	drawTexture(textTexture, &dst);
